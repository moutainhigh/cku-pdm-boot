      ...
      while (names.hasNext()) {
          ...
          try {
              ...
              if (className.equalsIgnoreCase("java.sql.Timestamp")) {
               ...
               //加类型判断逻辑，防止调用set方法出错  jiangj add
...
//              }
     ...
     * @throws InvocationTargetException
     */
    public static void copyMap2Bean_sql(Object bean, Map properties) throws
            IllegalAccessException, InvocationTargetException {
        // Do nothing unless both arguments have been specified
        if ( (bean == null) || (properties == null)) {
            return;
        ...
        while (names.hasNext()) {
            String name = (String) names.next();
            // Identify the property name and value(s) to be assigned
            if (name == null||name.equalsIgnoreCase("rownum_")) {
                continue;
            }
            Object value = properties.get(name);
            // 命名应该大小写应该敏感(否则取不到对象的属性)
            ...
            try {
				...
				buffer.append(name);
				int count = buffer.indexOf("_");
				while(count!=0){
					int num = buffer.indexOf("_",count);
					count = num+1;
					if(num!=-1){
						char ss = buffer.charAt(count);
						char ia = (char) (ss - 32);
						buffer.replace(count,count+1,ia+"");
					}
				}
				name = buffer.toString().replaceAll("_","");
