...
public class SysDictItemController {
	...
	@AutoLog(value = "获取字典数据id")
	@ApiOperation(value="获取字典数据id", notes="获取字典数据id")
	@RequestMapping(value = "/getType", method = RequestMethod.GET)
	public Result<?> getType(@RequestParam(name="dictname") String dictname,HttpServletRequest req) {
		return Result.OK(sysDictService.getType(dictname));
	}
	...
	public Result<List<SysDictItem>> getDictItems(@PathVariable String dictCode, HttpServletRequest request) {
		log.info(" dictCode : "+ dictCode);
		Result<List<SysDictItem>> result = new Result<List<SysDictItem>>();
		List<SysDictItem> ls = null;
		result.setResult(sysDictItemService.queryDictItemsByCode(dictCode));
		return result;
	}
	...
	@RequestMapping(value = "/addByCode", method = RequestMethod.POST)
	@CacheEvict(value= CacheConstant.SYS_DICT_CACHE, allEntries=true)
	public Result<SysDictItem> addbycode(@RequestBody SysDictItem sysDictItem) {
		Result<SysDictItem> result = new Result<SysDictItem>();
		try {
			sysDictItem.setCreateTime(new Date());
			QueryWrapper<SysDict> queryWrapper=new QueryWrapper<SysDict>();
			SysDict o = sysDictService.getOne(queryWrapper.eq("dict_code",sysDictItem.getDictId()));
			if(o==null){
				result.error500("请检查dictId");
			}
			sysDictItem.setDictId(o.getId());
			sysDictItemService.save(sysDictItem);
			result.success("保存成功！");
		} catch (Exception e) {
			log.error(e.getMessage(),e);
